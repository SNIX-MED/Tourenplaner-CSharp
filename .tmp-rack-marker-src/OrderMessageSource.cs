...
/// <summary>
/// Adds the Palettenregal configurator source marker to Smartstore order message items.
/// This runs while the first order e-mail model is being created, before OrderPlacedEvent.
/// </summary>
internal sealed class RackConfigOrderMessageEvents : IConsumer
{
        ...
        CancellationToken cancelToken)
    {
        ...
        var isMarked = string.Equals(
            order.GenericAttributes?.Get<string>(RackConfigTracking.SourceKey),
            RackConfigTracking.SourceValue,
            StringComparison.Ordinal);

        List<RackConfigPendingMarker> matched = [];
        ...
        if (!isMarked)
        {
            ...
            var pending = RackConfigTracking.ReadPending(
                customer.GenericAttributes.Get<string>(RackConfigTracking.PendingKey));
        ...
        if (existing.Contains(RackConfigTracking.SourceValue, StringComparison.OrdinalIgnoreCase))
            return;
        ...
        var sourceBadge =
            ...
            $"Quelle: {RackConfigTracking.SourceValue}{lineInfo}</div>";

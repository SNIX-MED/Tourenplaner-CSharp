...
public class GawelaColorAdminController : AdminController
{
    ...
    public async Task<IActionResult> Configure(GawelaAssetAdminModel model)
    {
        ...
        if (product == null)
        {
            ModelState.AddModelError(nameof(model.ProductReference), "Produkt nicht gefunden.");
            return View(await BuildModelAsync(model, null));
        }
        ...
        foreach (var layer in model.Layers)
        {
            ...
            if (a == null)
            {
                ModelState.AddModelError(nameof(model.Layers), "Ein ausgewähltes Attribut gehört nicht zu diesem Produkt.");
                continue;
            }
            ...
            if (!allowedRals.Contains(layer.BaseRal)) ModelState.AddModelError(nameof(model.Layers), $"Basis-RAL {layer.BaseRal} ist in der RAL-Farbliste nicht hinterlegt.");
            if (!allowedRals.Contains(layer.DefaultRal)) ModelState.AddModelError(nameof(model.Layers), $"Fallback-RAL {layer.DefaultRal} ist in der RAL-Farbliste nicht hinterlegt.");
            ...
            try { if (layer.Mask != null) _assetStore.ValidateMask(layer.Mask); }
            catch (InvalidOperationException ex) { ModelState.AddModelError(nameof(model.Layers), ex.Message); }
        }

        try { if (model.BaseImage != null) _assetStore.ValidateBase(model.BaseImage); }
        catch (InvalidOperationException ex) { ModelState.AddModelError(nameof(model.BaseImage), ex.Message); }
        ...
        var config = new GawelaProductConfig
        {
            ...
            ThumbnailLabel = string.IsNullOrWhiteSpace(model.ThumbnailLabel) ? "Farbe konfigurieren" : model.ThumbnailLabel.Trim()
        };
        foreach (var l in model.Layers)
        {
            var a = attrs.First(x => x.Id == l.ProductVariantAttributeId);
            config.Layers.Add(new GawelaLayerConfig
            {
                Key = l.Key,
                Name = l.Name,
                ProductVariantAttributeId = a.Id,
                AttributeLabel = a.Name,
                AssetKind = "layer-" + l.Key,
                BaseRal = l.BaseRal,
                DefaultRal = l.DefaultRal
            });
        }

        await _assetStore.SaveConfigAsync(config);
        _assetStore.DeleteUnusedLayerMasks(product.Id, config.Layers.Select(x => x.Key));
        TempData["GawelaColor.Success"] = $"Konfigurator für {product.Sku} – {product.Name} wurde gespeichert ({config.Layers.Count} Ebene(n)).";
        return RedirectToAction(nameof(Configure), new { productId = product.Id, tab = "products" });
    }
    ...
    public async Task<IActionResult> SavePalette(GawelaAssetAdminModel model)
    {
        ...
        {
            var vm = await BuildModelAsync(new GawelaAssetAdminModel(), null);
            vm.ActiveTab = "colors";
            vm.Palette = model.Palette ?? vm.Palette;
        ...
        TempData["GawelaColor.Success"] = "RAL-Farbwerte wurden gespeichert. Die Änderungen gelten zentral für alle konfigurierten Produkte.";
    ...
    public IActionResult Delete(int productId)
    {
        ...
        TempData["GawelaColor.Success"] = $"Konfigurator für Produkt-ID {productId} wurde gelöscht.";
    ...
    private async Task<GawelaAssetAdminModel> BuildModelAsync(GawelaAssetAdminModel model, ProductLookupResult product)
    {
        ...
        if (product != null)
        {
            ...
            model.ProductReference = product.Sku;
            ...
            if (model.Layers.Count == 0)
            {
                ...
                if (cfg?.Layers?.Count > 0)
                {
                    ...
                    {
                        var match = l.ProductVariantAttributeId > 0
                            ? model.AvailableAttributes.FirstOrDefault(x => x.Id == l.ProductVariantAttributeId)
                            : model.AvailableAttributes.FirstOrDefault(x => NamesMatch(x.Name, l.AttributeLabel));
                        if (match != null)
                        {
                            model.Layers.Add(new GawelaLayerAdminModel
                            {
                                ...
                                BaseRal = l.BaseRal,
                                DefaultRal = l.DefaultRal,
                                HasExistingMask = _assetStore.Exists(product.Id, l.AssetKind)
                            });
                        }
                    }
        ...
        if (ids.Length > 0)
        {
            var ps = await _db.Products.AsNoTracking().Where(x => ids.Contains(x.Id)).Select(x => new { x.Id, x.Sku, x.Name }).ToListAsync();
            var map = ps.ToDictionary(x => x.Id);
            foreach (var id in ids)
            {
                map.TryGetValue(id, out var p);
                var cfg = _assetStore.LoadEffectiveConfig(id);
                model.ConfiguredProducts.Add(new ConfiguredProductModel
                {
                    ProductId = id,
                    Sku = p?.Sku ?? "(Produkt fehlt)",
                    ProductName = p?.Name ?? "",
                    HasBaseImage = _assetStore.Exists(id, "base"),
                    LayerCount = cfg?.Layers?.Count ?? 0,
                    IsLegacy = _assetStore.LoadConfig(id) == null,
                    IsComplete = _assetStore.IsComplete(id)
                });
            }
        }
        return model;
    ...
    private async Task<List<GawelaAttributeOptionModel>> GetAttributesAsync(int productId)
    {
        var rows = await _db.ProductVariantAttributes.AsNoTracking().Include(x => x.ProductAttribute)
            .Where(x => x.ProductId == productId).OrderBy(x => x.DisplayOrder).ToListAsync();
    ...
    private static string NormalizeTab(string value) => value?.Equals("colors", StringComparison.OrdinalIgnoreCase) == true ? "colors" : "products";

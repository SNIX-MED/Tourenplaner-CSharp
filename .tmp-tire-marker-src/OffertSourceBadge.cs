...
{
    public sealed class TireRackSourceBadgeModel
    {
    ...
    public class TireRackSourceBadgeViewComponent : SmartViewComponent
    {
        private readonly SmartDbContext _db;
        private readonly IShoppingCartService _shoppingCartService;

        public TireRackSourceBadgeViewComponent(SmartDbContext db, IShoppingCartService shoppingCartService)
        {
            _db = db;
        ...
        public async Task<IViewComponentResult> InvokeAsync(WishlistModel model)
        {
            ...
            var marked = cart.Items.Select(x => x.Item)
                .Where(x => x.ParentItemId == null && visibleItemIds.Contains(x.Id) && x.GenericAttributes != null &&
                    string.Equals(x.GenericAttributes.Get<string>(TireRackTracking.SourceKey), TireRackTracking.SourceValue, StringComparison.Ordinal))
                .ToList();

            if (marked.Count == 0)
                return Empty();

            var lines = marked.Select(x => x.GenericAttributes?.Get<string>(TireRackTracking.LineKey))
            ...
            return View(new TireRackSourceBadgeModel
            {
                MarkedPositions = marked.Count,
                MarkedQuantity = marked.Sum(x => Math.Max(1, x.GenericAttributes?.Get<int>(TireRackTracking.QuantityKey) ?? x.Quantity)),
                Lines = lines,
                Skus = skus
            });
        }
...
namespace Gawela.TireRackConfig.Filters
{
    ...
    public sealed class TireRackWishlistSourceFilter : IAsyncResultFilter
    {
        private const string SystemName = "Gawela.TireRackConfig";
        private readonly IWidgetProvider _widgetProvider;
        public TireRackWishlistSourceFilter(IWidgetProvider widgetProvider) => _widgetProvider = widgetProvider;
        ...
        public async Task OnResultExecutionAsync(ResultExecutingContext context, ResultExecutionDelegate next)
        {
            ...
            {
                _widgetProvider.RegisterWidget("wishlist_items_top", new ComponentWidget<TireRackSourceBadgeViewComponent>
                {
...
namespace Gawela.TireRackConfig
{
    ...
    internal sealed class WishlistTrackingStartup : StarterBase
    {
        public override void ConfigureServices(IServiceCollection services, IApplicationContext appContext)
            => services.Configure<MvcOptions>(o => o.Filters.Add<TireRackWishlistSourceFilter>());
    }
}

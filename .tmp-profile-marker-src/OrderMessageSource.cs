...
internal sealed class ProfileRackOrderMessageEvents : IConsumer
{
    public async Task HandleEventAsync(MessageModelPartCreatedEvent<OrderItem> message, SmartDbContext db, CancellationToken cancelToken)
    {
        ...
        var isMarked = string.Equals(order.GenericAttributes?.Get<string>(ProfileRackTracking.SourceKey), ProfileRackTracking.SourceValue, StringComparison.Ordinal);
        List<ProfileRackPendingMarker> matched = [];
        if (!isMarked)
        {
        ...
        if (existing.Contains(ProfileRackTracking.SourceValue, StringComparison.OrdinalIgnoreCase)) return;
        ...
        var sourceBadge = $"<div style=\"margin-top:8px;padding:6px 8px;border-left:3px solid #f28c00;background:#fff7ed;color:#7a4300;font-weight:700\">Quelle: {ProfileRackTracking.SourceValue}{lineInfo}</div>";
